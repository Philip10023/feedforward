...
feature 'sign-out' do
  scenario 'once user signs up, Sign Out link is visible in navbar' do
    ...
    expect(page).to have_link("FeedForward")
  ...
  scenario 'once user logs in, Sign Out link is visible in navbar' do
    beppy = User.create(
  ...
  scenario 'when user is signed in, clicking Sign Out ends the user session and
            returns to index' do
    jemp = User.create(
